...
- Python 3.14 or newer (`requires-python = ">=3.14,<4.0"` in `pyproject.toml`).
...
Install dependencies, including dev tools such as pytest, ruff, and mypy:

```console
$ uv sync --all-extras --dev
```

Run the development server with auto-reload. `--app app:app` is required —
...
These are the same commands CI runs in `.github/workflows/tests.yml`.
